...
import {
  ...
  firstIncompleteStep,
  formFromDraft,
  requirementsForActor,
  STEPS,
  type CompanyFormState,
  type StepId,
} from "./company-onboarding-progress";
...
export function CompanyOnboardingForm({ actor = "vendor", guest = false, onAuthenticated }: CompanyOnboardingFormProps) {
  ...
  function renderCompany(): React.ReactNode {
    return (
      <section className="onboarding-section-card" aria-labelledby="company-section-title">
  ...
  function renderAddress(): React.ReactNode {
    return <section className="onboarding-section-card" aria-labelledby="address-section-title"><h2 id="address-section-title" className="home-section-title" tabIndex={-1}>Address</h2>{field("Address line one", "addressLine1", "text", "address-line1")}{field("Address line two (optional)", "addressLine2", "text", "address-line2")}{field("City", "addressCity", "text", "address-level2")}{field("Region (optional)", "addressRegion", "text", "address-level1")}{field("Postal code", "addressPostalCode", "text", "postal-code")}{stepActions("contact")}</section>;
  }

  function renderContact(): React.ReactNode {
    return <section className="onboarding-section-card" aria-labelledby="contact-section-title"><h2 id="contact-section-title" className="home-section-title" tabIndex={-1}>Contact</h2>{field("Name", "contactName", "text", "name")}{field("Email", "contactEmail", "email", "email")}{field("Phone", "contactPhone", "tel", "tel")}{stepActions("identifiers")}</section>;
  }

  function renderIdentifiers(): React.ReactNode {
    return (
      <section className="onboarding-section-card" aria-labelledby="identifiers-section-title">
  ...
  function renderCapabilities(): React.ReactNode {
    return (
      <section className="onboarding-section-card" aria-labelledby="capabilities-section-title">
      ...
      <header className="onboarding-hero">
        ...
        <p className="company-onboarding-copy">
          {guest
            ...
            : "Save your details as you go. You can return to any section before submitting for review."}
        </p>
        <div className="onboarding-progress">
          <div className="onboarding-progress-copy">
            <span>Registration progress</span>
            <strong role="status" aria-live="polite">
              Step {steps.findIndex((item) => item.id === step) + 1} of {steps.length}: {steps.find((item) => item.id === step)?.label}
            </strong>
          </div>
        ...
        <ol>
          {steps.map((item) => <li key={item.id}>
            <button
              type="button"
              disabled={saving}
              data-current={step === item.id}
              data-state={step === item.id ? "current" : steps.findIndex((part) => part.id === item.id) < steps.findIndex((part) => part.id === step) ? "previous" : "upcoming"}
              aria-current={step === item.id ? "step" : undefined}
              onClick={() => guest ? setStep(item.id) : void continueTo(item.id)}
            >{item.label}</button>
          </li>)}
        </ol>
      </nav>
      <form className="company-onboarding-form" noValidate onSubmit={submitRegistration}>
        {step === "company" ? renderCompany() : null}
        {step === "address" ? renderAddress() : null}
        {step === "contact" ? renderContact() : null}
        {step === "identifiers" ? renderIdentifiers() : null}
        {step === "capabilities" ? renderCapabilities() : null}
