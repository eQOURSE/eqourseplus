...
Last completed FR: FR-REG-01 SMS-delivery completion — 2026-09-07 — Production SMS OTPs now use an HTTPS-only AmazeSMS-backed `SmsAdapter` selected explicitly by `SMS_PROVIDER=amazesms`; local development, CI and staging remain sandboxed. Sender, entity, template ID and exact DLT message text are configuration, while the auth key comes from Secret Manager. Only provider codes 100/150 succeed; every other HTTP, provider, network or timeout outcome becomes a sanitized 503. Logs retain the non-sensitive provider status code while excluding the request URL, OTP, auth key, recipient, caught error and raw response; Sentry also excludes the sensitive endpoint from outgoing spans and breadcrumbs. Real-Mongo coverage pins the existing email-success/SMS-failure ten-minute lockout without redesigning it. The currently approved Tutrain sender/template are used for controlled testing pending the `EQOURS`/eQOURSE DLT swap. 352 tests are green locally; lint and production build are clean. CI's pinned Node 22.23.1 check remains the authoritative gate.
...
- [x] FR-PUB-02 jobs pages (2026-09-08) — Built SSR `/jobs` listing and static `/jobs/[slug]` details from typed seeded sample jobs, with category/language query filters, JobPosting JSON-LD, canonical metadata, and a footer Jobs link. The routes are intentionally noindex and excluded from `RESOLVING_ROUTES`, so they remain out of the sitemap until FR-PRJ supplies real postings. Refined the UI with DESIGN.md token-based gradient/ambient treatment, glass surfaces, elevated card hover states, responsive grids, focus-visible controls, and reduced-motion handling. Added listing/detail content guards for currency, rate/salary/earnings/payment language, named payment/KYC providers, and non-date numbers; the listing guard was proven failing on the temporary `3` opportunity count and passing after its removal. Complete repository test suite passes: 8/8 tasks, 162/162 web tests.
...
FR-PUB-02 metadata follow-up (2026-09-04): both seeded job routes now declare page-level `robots.index: false`; sitemap and robots disallow behavior remain unchanged. Targeted tests were added and lint passes.
