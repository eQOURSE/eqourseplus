...
Last completed FR: FR-FND-02 / FR-REG-01 email-delivery completion — 2026-09-05 — Production email OTPs now use a Resend-backed `MailerAdapter` selected explicitly by `MAILER_PROVIDER=resend`; local development, CI and staging default to the sandbox adapter without credentials. The provider call has a five-second timeout and maps every HTTP, network or timeout failure to a sanitized 503 without logging or returning the OTP, API key or provider response. Real-Mongo coverage pins the existing failed-registration state: the record is written before delivery, an immediate retry returns the field-agnostic 409, and reclaim succeeds only after the ten-minute challenge expiry. SMS remains sandboxed pending TRAI DLT registration. 321 tests are green locally; lint and production build are clean. CI's pinned Node 22.23.1 check remains the authoritative gate.
...
- [x] FR-REG-01 freelancer sign-up (2026-08-05; email delivery completed 2026-09-05) — Dedicated dual-OTP API, sparse-unique migration, device-review flags, two-step `/register/freelancer` browser flow, and production Resend email OTPs. SMS remains sandboxed pending DLT registration; the persisted-before-send ten-minute failure lockout is pinned by a real-Mongo test.
