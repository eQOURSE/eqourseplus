...
**Phase 0 of 8 complete; Phase 1 is most of the way through. Done: FR-FND-01..06, FR-PUB-00/00A/00B, FR-PUB-01 home, FR-PUB-03 `/freelancers`, FR-PUB-04 `/vendors`. The shared public design system in `packages/ui` provides SSR-first frosted glass, a corrected convex-profile live-content SVG refraction primitive with automatic fallbacks and a three-element budget, DESIGN.md-only motion, light/dark themes on a value ladder with no FOUC, responsive tokens, the §13 Liquid Glass language, a post-load ambient canvas, and the GlassNav / GlassButton / GlassSegmentedControl / GlassSubstrate family with full bezels and five interaction states. Three public routes are live, server-rendered and crawlable at plus.eqourse.com with Section 18 SEO complete on each: home (Organization + WebSite JSON-LD), `/freelancers` and `/vendors` (FAQPage + BreadcrumbList each). Shared public chrome — `SiteNavigation`, `SiteFooter`, ambient/theme islands, arrow mark, JSON-LD serializer — is extracted and reused across all three. Disjoint `RESOLVING_ROUTES` / `UNBUILT_ROUTES` registries with page-existence tests make it impossible for a route to enter the sitemap before it resolves. Content-honesty guards are in CI and load-bearing: a claims allow-list (`500+`, `30+`, `9001`, `27001`), zero-digit guards on `/freelancers` and `/vendors`, and negative guards on currency, earnings, named providers, client logos, fee language and SLA timelines. Testimonials ship as a typed empty collection that renders nothing. 186 tests green, lint and build clean. Production Lighthouse: home 99/100/100/100, `/freelancers` 98/100/100/100, `/vendors` 100/100/100/100. Zero focal-refraction elements on any public page. Next: FR-PUB-05 `/about`, then FR-PUB-06 login/register and FR-SEO-01. FR-PUB-02 `/jobs` remains deliberately deferred.**
...
- Outstanding content: 2-3 real testimonials (name, role, company, consented quote) for apps/web/content/testimonials.ts. The component renders nothing until supplied; no code change needed. eqourse.com/clients_testimonials already publishes client testimonials — check whether any can be reused here with explicit consent and correct attribution before sourcing new ones.
- Google Search Console: create a separate property for plus.eqourse.com per Section 18, submit sitemap.xml, request indexing of the home route. Then add crawlable links from eqourse.com header/footer/homepage to the subdomain - that is what passes authority across, and without it the subdomain starts from zero.
...
- Stale merged branches safe to delete on the remote: `docs/FR-PUB-01-closeout`, `docs/FR-PUB-03-closeout`, `feat/FR-PUB-03-freelancers`, `chore/ci-paths-ignore-docs`, `feat/FR-PUB-04-vendors`.
- Minor SEO tidy: the footer links to `https://eqourse.com`, which self-canonicalises to `https://www.eqourse.com/`. `/vendors` already links the case-study page on the `www` host. Align the footer to `www` on the next public-route FR so all outbound links hit the canonical host.
