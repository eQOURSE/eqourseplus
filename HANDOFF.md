...
- SPEC.md (v2.12, ~60 FR IDs w/ acceptance criteria, Part A core + Part B global/build guide, Section 19.2
...
**Phase 0 of 8 is complete. Phase 1 is complete except FR-SEO-01 and FR-PUB-02, both deliberately deferred. Phase 2 is in progress, with FR-REG-01 and its FR-REG-02A session-transport prerequisite complete. Completed foundation and public work: FR-FND-01..06, FR-PUB-00/00A/00B, FR-PUB-01 home, FR-PUB-03 `/freelancers`, FR-PUB-04 `/vendors`, FR-PUB-05 `/about`, and FR-PUB-06 login/register. FR-REG-01 provides dual-OTP freelancer registration, sparse-unique phone/PAN handling, device-fingerprint review flags, and the two-step `/register/freelancer` flow. FR-REG-02A adds the authenticated API session read and same-origin web handlers for registration verification, sign-in verification, refresh, logout and session read. Tokens remain server-side in `__Host-` httpOnly cookies; session reads refresh and retry once, explicit sign-out revokes then clears, and refresh failures leave cookies untouched so concurrent tabs cannot erase a winning rotation. Registration and OTP request endpoints remain direct browser-to-API calls where the real client IP is required. Four public indexable routes remain live and server-rendered: `/`, `/freelancers`, `/vendors`, and `/about`; the four account-entry routes remain noindex. 308 tests are green locally; lint and production build are clean, with CI's pinned Node 22.23.1 check authoritative. Next: FR-REG-02 profile wizard. FR-SEO-01 and FR-PUB-02 remain deliberately deferred.**
...
Browser-to-API calls for anything fingerprinted. The device fingerprint is derived server-side from the request’s IP, user-agent and accept-language. Any route that proxies a fingerprinted call through Next.js makes every request arrive with the web server’s identity, giving every user an identical fingerprint and rendering duplicate detection useless. `register/request` and `otp/request` therefore remain direct browser calls behind the API's explicit CORS allowlist; verification, refresh, logout and authenticated traffic use same-origin web handlers.

Session transport and concurrent refresh. The web origin owns `__Host-` httpOnly/Secure/SameSite=Lax cookies; their Max-Age values and the API token TTLs consume shared constants. Unsafe handlers require an exact `APP_URL` Origin. `Sec-Fetch-Site` is defence in depth: reject `same-site` and `cross-site` when present, but permit absence for older browsers. A failed refresh returns 401 without any `Set-Cookie`, so a concurrent losing tab cannot clear the rotated cookies installed by the winner. The loser can still observe one transient 401; a subsequent request succeeds with the winner's cookies. Only explicit sign-out clears both cookies, after API revocation. A headed Chromium check confirmed that HTTP localhost accepts and resends a Secure `__Host-` cookie.
...
- `/register/freelancer` cannot work on production until the FR-FND-05 Cloud Run configuration fix reaches production. The form calls the API directly from the browser and there is no healthy production API to call. The page renders and the form validates, but submission will fail until the manually approved production deployment completes.
...
- The FR-FND-05 Cloud Run startup failure was caused by the workflow omitting required `CORS_ORIGINS` while the production image correctly fails closed without it; it was not a `SENTRY_DSN` IAM defect. The workflow now sets staging to `http://localhost:3000` and production to `https://plus.eqourse.com` as non-secret runtime configuration and checks that the variable is present before deployment. Staging deliberately excludes per-branch Vercel preview origins, so previews cannot make browser-originated registration or OTP-request calls until a stable preview origin exists.
...
- Production API Cloud Run deploy remains protected by manual approval on every merge to main. Do not approve it merely to clear the deployment-review dialog; review and intentionally promote the exact staging-verified commit image. Leaving it pending is preferred over rejecting, which marks the run failed.
