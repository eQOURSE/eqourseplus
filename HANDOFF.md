...
- Home page Lighthouse Performance moved from a clean 100 to 96/99/99 across three samples after the FR-PUB-03 shared-chrome extraction. Median 99, well above the >=90 gate, and no repeatable regression was found - treated as run-to-run noise. Re-check on the next public-route FR; if it trends down rather than bouncing, investigate the shared chrome bundle.
- Public nav and the home contextual CTA link to /freelancers#top rather than /freelancers. Crawlers strip the fragment so link equity is unaffected, but the anchor does nothing unless an id="top" element exists. Tidy when FR-PUB-04 adds /vendors to the same nav.
- LAUNCH GATE (production verified clean 2026-07-27): plus.eqourse.com must never ship with Vercel Deployment Protection enabled for Production. Protection injects x-robots-tag: noindex and 302s crawlers to vercel.com/sso-api, making the site permanently unindexable and silently voiding all Section 18 SEO work. Re-verify after any Vercel settings change with curl -I on the production URL: expect 200, no redirect to vercel.com/sso-api, and no x-robots-tag: noindex. Protected previews legitimately score SEO 66 for this reason; production scores 100.
- Outstanding content: 2-3 real testimonials (name, role, company, consented quote) for apps/web/content/testimonials.ts. The component renders nothing until supplied; no code change needed.
