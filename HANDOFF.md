...
**Phase 0 of 8 complete; Phase 1 is nearly done. Done: FR-FND-01..06, FR-PUB-00/00A/00B, FR-PUB-01 home, FR-PUB-03 `/freelancers`, FR-PUB-04 `/vendors`, FR-PUB-05 `/about`. The shared public design system in `packages/ui` provides SSR-first frosted glass, a corrected convex-profile live-content SVG refraction primitive with automatic fallbacks and a three-element budget, DESIGN.md-only motion, light/dark themes on a value ladder with no FOUC, responsive tokens, the §13 Liquid Glass language, a post-load ambient canvas, and the GlassNav / GlassButton / GlassSegmentedControl / GlassSubstrate family with full bezels and five interaction states. Four public routes are live, server-rendered and crawlable at plus.eqourse.com with Section 18 SEO complete on each: home (`Organization` + `WebSite`), `/freelancers` and `/vendors` (`FAQPage` + `BreadcrumbList` each), and `/about` (`Organization` + `BreadcrumbList`). Entity identity is now centralised in `apps/web/app/site-structured-data.ts`: one canonical `Organization` node at `https://plus.eqourse.com/#organization`, described on `/` and referenced by identical `@id` from `/about`, with `parentOrganization` bound to `https://www.eqourse.com/#organization` — the reciprocal half of the `subOrganization` array already published on eqourse.com. The parent is named "eQOURSE" at brand level, not by legal entity. Public routes are governed by three mutually disjoint registries — `RESOLVING_ROUTES`, `EXCLUDED_ROUTES`, `UNBUILT_ROUTES` — with page-existence assertions in both directions, so a route cannot enter the sitemap before it resolves and a deliberately-excluded route (`/design-system`) cannot silently lose its guard. Content-honesty guards are in CI and load-bearing: a claims allow-list (`500+`, `30+`, `9001`, `27001`) applied at exact-set equality on home and `/about`, zero-digit guards on `/freelancers` and `/vendors`, and negative guards on currency, earnings, named providers, client logos, fee language, SLA timelines, registration identifiers and street addresses. Testimonials ship as a typed empty collection that renders nothing. 202 tests green, lint and build clean. Production Lighthouse: home 99/100/100/100, `/freelancers` 98/100/100/100, `/vendors` 100/100/100/100; `/about` not yet measured. Zero focal-refraction elements on any public page. Next: FR-PUB-06 login/register, then FR-SEO-01. FR-PUB-02 `/jobs` remains deliberately deferred.**
...
- LAUNCH GATE (production verified clean 2026-07-29): plus.eqourse.com must never ship with Vercel Deployment Protection enabled for Production. Protection injects x-robots-tag: noindex and 302s crawlers to vercel.com/sso-api, making the site permanently unindexable and silently voiding all Section 18 SEO work. Re-verify after any Vercel settings change with curl -I on the production URL: expect 200, no redirect to vercel.com/sso-api, and no x-robots-tag: noindex. Protected previews legitimately score SEO 66 for this reason; production scores 100.
...
- Stale merged branches safe to delete on the remote: `chore/ci-paths-ignore-docs`, `chore/gsc-verification`, `docs/FR-PUB-01-closeout`, `docs/FR-PUB-03-closeout`, `docs/FR-PUB-04-closeout`, `feat/FR-FND-04-ci-workflow`, `feat/FR-FND-05-cloud-run-deploy`, `feat/FR-FND-05-jwt-secret`, `feat/FR-PUB-04-vendors`, `feat/FR-PUB-05-about`. (Verified against the remote 2026-08-01; `feat/FR-PUB-03-freelancers` was already deleted.)
- The footer `www` alignment is DONE (FR-PUB-05): all outbound links now target `https://www.eqourse.com/`.
- `/about` production Lighthouse not yet recorded. The other three public routes have scores; this one does not. Remember the focal-refraction rule when measuring: scroll into view and wait 15 s or the run measures the frosted fallback.
