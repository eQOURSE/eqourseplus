# eQOURSE+ — SaaS Requirements Specification (SPEC.md) v2.25 — Global Edition
...
|---|---|---|
| FR-REG-01 | [P1] Freelancer sign-up is verified by email OTP only, with country selection; phone verification is not performed (phone verification removed v2.18). Duplicates are hard-blocked by phone and PAN; device fingerprint is recorded and flagged for verifier review, never auto-blocked. The phone number is still collected, validated as E.164 and held to the sparse unique constraint, and duplicate phones remain hard-blocked. Identity is established by FR-REG-03 and FR-REG-04, which are materially stronger signals than SMS receipt and are not restricted to a single country. The registration flow never populates `phoneVerifiedAt`; legacy values are ignored and retained without migration. `register/request` still reports which channels it issued and always returns `["email"]`. | No second account with the same phone or PAN. Only email OTP is issued and required, the SMS adapter is not called, and new registration records do not populate `phoneVerifiedAt`. A supplied phone OTP is rejected. Existing `phoneVerifiedAt` and `phoneOtpChallenge` values are never read and remain stored without migration or clearing. A repeated device fingerprint creates a review flag visible to FR-REG-07 and does not by itself prevent sign-up. |
...
**profiles** — one document per freelancer profile.
```
{
  _id: ObjectId,
  ...
  experience?: {
    totalMonths?: number,
    entries?: Array<{
      organization?: string, title?: string, startDate?: Date, endDate?: Date, summary?: string
    }>
  },
  samples?: Array<{
  ...
  availability?: {
    availableFrom?: Date, weeklyHours?: number, timeZone?: string
  },
  rate?: {
    amountMinor?: number,                      // integer minor units only, never a float
    currencyCode?: string,
    unit?: "HOUR"
  },
  createdAt: Date, updatedAt: Date
...
## 22. Foundation & Public-Site Requirements (Phase 0–1 FR IDs — added v2.1; FR-PUB-00 added v2.5; FR-PUB-00A added v2.6; FR-PUB-00-HOME-DESIGN added v2.25)
...
| FR-PUB-00-HOME-DESIGN | [P1] Home-page redesign authorization (added v2.25; **created by Kartik for this implementation**). This FR permits implementation of a redesigned public eQOURSE+ home page at `/`, including changes to its visual presentation, layout, and component composition. The redesign must retain the home-page content required by FR-PUB-01 and comply with FR-PUB-00, FR-PUB-00A, FR-PUB-00B, FR-PUB-00C, DESIGN.md, Section 18 SEO standards, responsive behavior, `prefers-reduced-motion`, and WCAG-AA. This authorization does not add product features, API endpoints, collections, or fields. | The redesigned `/` home page is implemented and preserves the FR-PUB-01 sections; it is SSR-rendered, responsive across mobile/tablet/desktop, keyboard accessible, respects reduced-motion preferences, and meets the existing Lighthouse and metadata/canonical requirements. No new product behavior or data contract is introduced. |
...
Ordering rule: FND-01→06 strictly sequential; PUB-00→00A→00B→00C→00-HOME-DESIGN→01..06 then FR-SEO-01. FR-SEO-01 must not begin before FR-FND-01/05 are done.
