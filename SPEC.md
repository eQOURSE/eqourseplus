# eQOURSE+ — SaaS Requirements Specification (SPEC.md) v2.16 — Global Edition
...
|---|---|---|
| FR-REG-01 | [P1] Freelancer sign-up with email verification and country selection; duplicates hard-blocked by phone and PAN; device fingerprint recorded and flagged for verifier review, never auto-blocked. **Phone verification is configurable (added v2.14) and defaults to off.** The phone number is always collected, validated as E.164 and held to the sparse unique constraint, but ownership is not proven at sign-up. Identity is established by FR-REG-03 and FR-REG-04, which are materially stronger signals than SMS receipt and are not restricted to a single country. When `PHONE_VERIFICATION_REQUIRED` is enabled the dual OTP flow applies unchanged. `register/request` reports which channels it issued so the client never needs a second copy of this configuration. | No second account with the same phone or PAN. With phone verification disabled, only email OTP is issued and required, the SMS adapter is not called, and `phoneVerifiedAt` remains null; with it enabled, both email and phone OTPs are issued and required. A supplied phone OTP is rejected when phone verification is disabled. A repeated device fingerprint creates a review flag visible to FR-REG-07 and does not by itself prevent sign-up. |
