# eQOURSE+ — SaaS Requirements Specification (SPEC.md) v2.30 — Global Edition
...
| FR-REG-02C | [P1] Sign-in interface (added v2.23; routing revised v2.30): a working `/login` signs an existing account in by email OTP, supports sign-out, and links to `/register` in one action; `/register` links back to `/login` in one action. It consumes the existing `POST /api/v1/auth/otp/request`, `POST /api/v1/auth/otp/verify` and `POST /api/v1/auth/logout` under FR-REG-02A's normative route matrix and session transport; no new API endpoint is introduced. Immediately after a new account completes email verification in registration, it continues to `/profile`, preserving the registrant's active intent to begin the profile wizard; this first-registration continuation is deliberately distinct from the destination of a later sign-in. After any later sign-in, the destination is determined in this order: first, an existing vendor or client record owned by the user routes to that company's role-resolved `/dashboard` home under FR-REG-17 or FR-REG-18, including when the company is still `DRAFT`; when both records exist, vendor wins as the explicit deterministic tie-break because it preserves the established vendor-first routing behaviour, and any future company switcher requires its own FR. Second, if the user owns neither record, a role assignment that grants access to an internal surface routes the user to that surface as defined by its owning FR. Otherwise, the user reaches the specialist `/dashboard` under FR-REG-16, where a visible Register a company action links to `/register`. Owned company records remain the first determinant, and `roleAssignments` never selects or overrides a company destination: a stale company role such as `VENDOR` without its corresponding record must not route the user into a company wizard that creates an empty draft record for a company they do not have. The middle step applies only to roles granting internal surfaces, because internal roles own no company by design. The specialist home is a valid final fallback because Section 19.2 guarantees every account an owning profile; the registration chooser remains reachable but is no longer a returning sign-in destination. Unknown and known email addresses have the same visible OTP-request outcome. The platform has no passwords: the interface neither collects nor suggests one or offers password recovery. `/login` retains `robots.index:false` and remains excluded from the sitemap. Registration, the profile wizard, role-assignment management and implementation of internal surfaces are out of scope. | Immediately after first-time registration email verification, the account reaches `/profile`; on a later sign-in, a vendor or client owner reaches that company's `/dashboard`, with vendor winning if both records exist, and a `DRAFT` company home's Next action resumes its registration wizard at the first incomplete section. A user owning neither company record but holding a role that grants an internal surface reaches that surface; a stale company role without its corresponding record never selects a company wizard; and every other user reaches the specialist `/dashboard` with a visible route to company registration. Sign-out ends the session; no token is readable from JavaScript; an unknown address produces the same visible OTP-request outcome as a known one; `/login` and `/register` each link to the other; and the "not open yet" copy is gone. |
...
| FR-REG-16 | [P1] Specialist home on the shared role-resolved `/dashboard` route for freelancers, SMEs, QA reviewers and technical experts. The live Next action panel shows one sentence and one state-appropriate button derived from the authoritative profile state and FR-REG-02B's computed `completionPercentage`, never from a stored duplicate; the live Profile panel shows completion percentage, state and a continue or edit link. Identity verification, Skill assessments, Available work, Active assignments, Quality and feedback, and Earnings are pending panels governed by the role-home empty-state rule below. Specialist-role nuance changes the assessments and work eventually shown, not the panel set. The home visibly offers Register a company linking to `/register`, so the sign-in fallback does not hide vendor or client onboarding. `/dashboard` is authenticated, carries page metadata `robots.index:false`, is a member of `EXCLUDED_ROUTES`, is absent from `RESOLVING_ROUTES` and the sitemap, and is not added to the robots disallow list so crawlers can read its `noindex`. | A returning account with no owned company and no internal-surface role reaches `/dashboard`; its Next action and Profile values match the current profile response, no pending panel renders invented user data, Register a company reaches `/register`, an unauthenticated request cannot view the home, and route-registry tests prove `/dashboard` is in `EXCLUDED_ROUTES` and absent from the sitemap. |
| FR-REG-17 | [P1] Vendor-agency home on the shared role-resolved `/dashboard` route. The live Next action panel shows one sentence and one state-appropriate button derived from authoritative vendor state and existing vendor data, never from a stored duplicate; when state is `DRAFT`, it states that registration is incomplete and its button resumes `/register/vendor` at the first incomplete section. Accreditation shows company state, stage and outstanding items; Company profile shows legal details, documents and an edit link; Capabilities shows the vendor's selected taxonomy areas. Those four panels are live. Team, Work orders, Delivery, Quality and Settlement are pending panels governed by the role-home empty-state rule below. The shared route retains FR-REG-16's authenticated, `robots.index:false`, `EXCLUDED_ROUTES`, no-sitemap and no-robots-disallow contract. | A vendor owner signs in to `/dashboard` rather than the registration wizard; authoritative company data populates every live panel; a `DRAFT` vendor can resume the wizard at its first incomplete section from Next action; submitted and reviewed vendors are not sent back through completed registration; no pending panel renders invented user data; and an unauthenticated request cannot view the home. |
| FR-REG-18 | [P1] Client home on the shared role-resolved `/dashboard` route. The live Next action panel shows one sentence and one state-appropriate button derived from authoritative client state and existing client data, never from a stored duplicate; when state is `DRAFT`, it states that registration is incomplete and its button resumes `/register/client` at the first incomplete section. Account shows company state, documents and an edit link and is live. Projects, Assigned talent and teams, Delivery telemetry, Review queue, Milestones and billing, and Agreements are pending panels governed by the role-home empty-state rule below. The shared route retains FR-REG-16's authenticated, `robots.index:false`, `EXCLUDED_ROUTES`, no-sitemap and no-robots-disallow contract. | A client owner signs in to `/dashboard` rather than the registration wizard; authoritative company data populates every live panel; a `DRAFT` client can resume the wizard at its first incomplete section from Next action; submitted and reviewed clients are not sent back through completed registration; no pending panel renders invented user data; and an unauthenticated request cannot view the home. |
...
**Role-home presentation rule (normative for FR-REG-16, FR-REG-17 and FR-REG-18).** Each role home uses one shared shell and a role-resolved panel inventory. Every Next action is computed on read from authoritative profile or company state and other already-owned source data and is never persisted; storing a second copy would allow the fact shown to the user to drift from the source. A panel whose underlying feature is not yet built renders an empty state describing in one line what will appear there and, where an implemented destination exists, one relevant CTA. It never renders placeholder, sample or illustrative values. A figure inside a signed-in user's own home is read as that person's or company's data, so a fabricated figure is a false statement to a real person. This rule deliberately carries the lesson from the invented public operating metrics removed in PR #85 into the less visible and therefore harder-to-audit authenticated surface.
...
SPECIALIST (Freelancer, SME, QA Reviewer, Expert): register → verify → country → 5-step profile wizard → shared
role-resolved `/dashboard` home (FR-REG-16) → per-country KYC step (live status chips) → payout setup → e-sign →
under-review → Test Center (proctoring pre-check) → proctored test → result (badge/tier or cooldown) → Workbench
(task center, SOP side panel, timer, rework banner). Role nuance changes dashboard content, not its panel set.
VENDOR: company register → KYB checklist → signatory KYC → capability → MSA → shared role-resolved `/dashboard`
home (FR-REG-17) → Team mgmt
...
CLIENT: company register → verification → shared role-resolved `/dashboard` home (FR-REG-18) → project creation →
talent and team assignment → delivery monitoring → review queue → milestone approval → billing and agreements.
...
Assessment showcase seed (FR-TST-01): EQOURSE / AI Data Services / Image Annotation Quality Assurance / null; EQOURSE / Content Services / Multilingual Content Quality Review / null. These complement the existing Bounding Box node for the global buyer demo; the three FR-FND-03 seed rows stay unchanged.
...
```
{
  ...
  assessmentBadges: Array<{ taxonomySlug: string, tier: "BRONZE" | "SILVER" | "GOLD", scorePercent: number, awardedAt: Date }>,
  createdAt: Date, updatedAt: Date
}
```
...
**Assessment collections (FR-TST-01/02-lite/05/06)** — keyed by active `skillTaxonomy.slug`.

`tests`: one document per category with `taxonomySlug` (unique), `timeLimitSeconds`, `questionCount`, `passThresholdPercent`, `cooldownDays` (default 14), `maxAttempts` (default 2), `tierBands` (`silverMinPercent`, `goldMinPercent`), `questions`, `createdAt`, `updatedAt`. Each question has a stable `id`, `kind` (`MCQ` for this slice), `status` (`DRAFT` or `APPROVED`), `prompt`, `difficulty` (`EASY`/`MEDIUM`/`HARD`), `options` (stable `id` and `text`), and `correctOptionId`. Only approved questions are served. Admin configuration validates category existence and ordered score bands; question promotion requires a human review. Answer keys are never returned by candidate endpoints.

`testAttempts`: `userId`, `taxonomySlug`, `attemptNumber`, `status` (`IN_PROGRESS`/`PASSED`/`FAILED`/`UNDER_REVIEW`/`EXPIRED`), `startedAt`, `expiresAt`, `submittedAt`, `passThresholdPercent` and `tierBands` snapshots, `items` (question ID, prompt and shuffled option IDs/text plus server-only correct option ID), `answers`, `scorePercent`, `violations` (kind `FULLSCREEN_EXIT`/`TAB_SWITCH`/`WINDOW_BLUR`, server timestamp), `tier`, `createdAt`, `updatedAt`. Unique `(userId, taxonomySlug, attemptNumber)`; index `(userId, taxonomySlug, startedAt)`. The server selects, shuffles, timestamps, scores, and enforces the deadline and attempt limits. A flagged submission is `UNDER_REVIEW` regardless of score and earns no badge until human review.

`profiles.assessmentBadges` is keyed by `taxonomySlug` with `tier`, `scorePercent`, and `awardedAt`. An unflagged pass writes the badge on the existing profile; the assessment service exposes an award method for the FR-REG-06 state machine to call. This slice does not implement a profile-state transition. Candidate routes are limited to their own attempts; admin configuration and answer keys require super-admin RBAC; verifier reports are business-unit scoped.
